			...
			for (int j = 0; j < datasetfilenames.length;j++) {
				...
				try {
					...
					for (int k = 0; k < 10; k++) { // Runs each test 10 times, to get the average time taken
							...
							NearestNeighFileBased.main(new String[]{agents[1], datasetfilenames[j] + ".txt", absolutefilepath + datasetfilenames[j].toLowerCase() + testfilenames[i] + ".in", absolutefilepath + datasetfilenames[j].toLowerCase() + testfilenames[i] + ".out"}); // Call the implementation
